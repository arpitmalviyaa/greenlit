...
type DocRow = { id: string; doc_kind: string; deal_type: string; title: string | null; status: string; chunk_count: number; created_at: string };
...
type Staged = { file: File; doc_kind: string; deal_type: string; title: string; founder_note: string; source_note: string; state: "idle" | "uploading" | "done" | "error"; msg?: string };
  ...
  const add = useCallback((files: FileList | null) => {
    ...
    setItems((prev) => [
      ...
      ...Array.from(files).map((file) => ({
        file, doc_kind: "contract", deal_type: "other",
    ...
    <div className="overflow-x-auto">
      <table className="w-full text-left text-sm">
        <thead className="text-xs uppercase text-zinc-500">
          <tr><th className="py-2">Title</th><th>Kind</th><th>Deal</th><th>Chunks</th><th>Status</th></tr>
        </thead>
        <tbody>
          {docs.map((d) => (
          ...
          {!docs.length && <tr><td colSpan={5} className="py-6 text-center text-zinc-500">No documents yet.</td></tr>}
        </tbody>
      </table>
    </div>
  );
...
function DocDetail({ id, onBack }: { id: string; onBack: () => void }) {
  ...
  const load = useCallback(async () => {
    ...
    if (res.ok) {
      const body = await res.json() as { document: { title: string | null }; chunks: Chunk[] };
...
function NoteView() {
  ...
  async function save() {
    ...
    const res = await fetch("/api/admin/corpus", {
      ...
      body: JSON.stringify({ noteText: text, deal_type: dealType, title }),
    });
    setBusy(false);
